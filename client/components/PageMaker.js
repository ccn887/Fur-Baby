      ...
      <div className='app'>
        ...
        <div className='drop-wrapper'>
          ...
          <Dropzone
            multiple={true}
            accept="image/*"
            onDrop={(files) => props.addImage(files[0], 'eugrrptb', currFurId, pageid)}>
            <p className="med-text">Drop an image or click to select a file to upload with a frosty filter.</p>
          </Dropzone>

          <Dropzone
            multiple={true}
            accept="image/*"
            onDrop={(files) => props.addImage(files[0], 'egwbtqid', currFurId, pageid)}>
            <p className="med-text">Drop an image or click to select a file to upload with a springtime filter.</p>
          </Dropzone>
